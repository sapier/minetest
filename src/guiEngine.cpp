	...
	m_cloud()
{
	...
	core::rect<s32> rect(0, 0, fe->getTextWidth(t), fe->getTextHeight());
	rect += v2s32(4, 0);
...
void GUIEngine::run()
{
	...
	unsigned int text_height = fe->getTextHeight();

	while(m_device->run() && (!m_startgame) && (!m_kill)) {

		//check if we need to update the "upper left corner"-text
		if (text_height != fe->getTextHeight()) {
			updateTopLeftTextSize();
			text_height = fe->getTextHeight();
		}
...
void GUIEngine::updateTopLeftTextSize()
{
	...
	core::rect<s32> rect(0, 0, fe->getTextWidth(text), fe->getTextHeight());
		rect += v2s32(4, 0);
