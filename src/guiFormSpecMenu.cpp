...
static gui::IGUIFont *select_font_by_line_height(double target_line_height)
{
	...
	unsigned int hireq = ceil(target_line_height
				...
				/ g_settings->getFloat("gui_scaling"));
	unsigned int lohgt = font_line_height(fe->getFont(loreq));
	unsigned int hihgt = font_line_height(fe->getFont(hireq));
	while(hireq - loreq > 1 && lohgt != hihgt) {
		unsigned int nureq = (loreq + hireq) >> 1;
		unsigned int nuhgt = font_line_height(fe->getFont(nureq));
	...
	return fe->getFont(target_line_height - lohgt < hihgt - target_line_height ? loreq : hireq);
}
